...
int main()
{
    ...
    for(int steps=1; steps<=numberOfSteps; steps++)
    {
        ...
        for(int i=0; i<numberOfChromosomes; i++)
        {
            ...
            int cr=binarySearch(interval, numberOfChromosomes, uList[i])+1;
            while(cr==maxChromosomeFit)
            {
                double randomDouble = distributionDouble(gen);
                cr=binarySearch(interval,numberOfChromosomes,randomDouble)+1;
            }
            selection[i]=cr;
            /*if(cr==maxChromosomeFit && cr>1)
                selection[i]=cr-1;
            else if(cr==maxChromosomeFit && cr==1)
                selection[i]=cr+1;
            else selection[i]=cr; //pentru [qi,qi+1) selectez cromozomul i+1
                */
        }
        ...
        if(contor%2==0)
        {
            for(int i=0; i<contor; i+=2)
            {
                ...
                int randomNumber1=distributionInt(gen);
                int randomNumber2=distributionInt(gen);
                if(steps==1) //afisez doar prima etapa
                    if(randomNumber1<randomNumber2)
                        fout<<selectedChromosomes[crossoverSelected[i]-1]<<"  +  "<<selectedChromosomes[crossoverSelected[i+1]-1]<<" intre punctele "<<randomNumber1<<" si "<<randomNumber2<<'\n';
                    else
                        fout<<selectedChromosomes[crossoverSelected[i]-1]<<"  +  "<<selectedChromosomes[crossoverSelected[i+1]-1]<<" intre punctele "<<randomNumber2<<" si "<<randomNumber1<<'\n';
                //RECOMBINAREA celor 2 cromozomi selectati
                crossover2Chromosomes(selectedChromosomes, crossoverSelected, randomNumber1, randomNumber2, length, i);
                if(steps==1) //afisez doar prima etapa
        ...
        else
        {
            ...
            //pe ultimul il combin cu ultimul combinat anterior cu alt cromozom
            int randomNumber1=distributionInt(gen);
            int randomNumber2=distributionInt(gen);
            if(steps==1)
            {
                if(randomNumber1<randomNumber2)
                    fout<<selectedChromosomes[crossoverSelected[contor-2]-1]<<"  +  "<<selectedChromosomes[crossoverSelected[contor-1]-1]<<" intre punctele "<<randomNumber1<<" si "<<randomNumber2<<'\n';
                else
                    fout<<selectedChromosomes[crossoverSelected[contor-2]-1]<<"  +  "<<selectedChromosomes[crossoverSelected[contor-1]-1]<<" intre punctele "<<randomNumber2<<" si "<<randomNumber1<<'\n';
            }
            crossover2Chromosomes(selectedChromosomes, crossoverSelected, randomNumber1, randomNumber2, length, contor-2);
            if(steps==1) //afisez doar prima etapa
            {
                fout<<"Combinam si cu cromozomul "<<crossoverSelected[contor-1]<<". Rezultat   "<<selectedChromosomes[crossoverSelected[contor-2]-1]<<"      "<<selectedChromosomes[crossoverSelected[contor-1]-1]<<'\n';
                fout<<'\n';
            }
        }
